...
public class OrreryDrawable extends LayerDrawable {
	...
	private void setEarthPosition(float rotationEarth)
	{
		...
		setLayerInset(
				...
				(int) (SPACE_HEIGHT - earthCenter.x - RADIUS_EARTH),
				(int) (SPACE_HEIGHT - earthCenter.y - RADIUS_EARTH));
	...
	private void setMoonPosition(float rotationMoon)
	{
		...
		setLayerInset(
				...
				(int) (SPACE_HEIGHT - moonCenter.x - RADIUS_MOON),
				(int) (SPACE_HEIGHT - moonCenter.y - RADIUS_MOON));
	...
	private Point getEarthCenter()
	{
		...
		earthCenter.x = 
				(int) (SPACE_HEIGHT/2 + ORBIT_EARTH*Math.sin(rotationEarth));
		earthCenter.y = 
				(int) (SPACE_HEIGHT/2 + ORBIT_EARTH*Math.cos(rotationEarth));
		return earthCenter;
	}
